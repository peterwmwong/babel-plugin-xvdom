...
let LAST_PATH_ID = 0;
class Path {
  static moreDesirable(a, b) {
    return a.isMoreDesirableThan(b) ? a : b;
  }
  ...
  get shouldBeSaved() {
    return this.children.size > (this.isHardReferenced ? 0 : 1) || this.referencingDynamics.size > 1;
  }

  get id() {
    const referenceIndicator = this.isHardReferenced ? '*' : this.shouldBeSaved ? '^' : '';
  ...
  isMoreDesirableThan(b) {
    ...
    const bEl = b.el;
    if (LOGGING.path) {
      log('path', 'isMoreDesirableThan:', `length: ${ a.id }(${ a.length }) < ${ b.id }(${ b.length })`, `elNumDynamicDescendants: ${ aEl.numContainedDynamics } < ${ bEl.numContainedDynamics }`, `numDynamicProps: ${ aEl.numDynamicProps } < ${ bEl.numDynamicProps }`);
    }
    return 0 >= (Math.sign(a.length - b.length) || Math.sign(bEl.numContainedDynamics - aEl.numContainedDynamics) || Math.sign(bEl.numDynamicProps - aEl.numDynamicProps));
  }
  ...
  finalize(dynamic) {
    return logFunc('path', `finalize ${ this.id }`, () => {
  ...
  removeReference(childPath) {
    logFunc('path', `removeReference ${ this.id } ${ this.shouldBeSaved }`, () => {
      this.children.delete(childPath);
    }, () => `-> ${ this.shouldBeSaved }`);
  }

  addReference(childPath) {
    logFunc('path', `addReference ${ this.id } ${ this.shouldBeSaved }`, () => {
...
function getPath(el, ignoreEl, isHardReferenced) {
  return logFunc('path', `getPath ${ el.tag } ${ ignoreEl ? `ignoring ${ ignoreEl.tag }` : '' }`, () => {
    ...
    if (index === 0) {
      parentPath = ignoreEl === nextSibling ? getPath(parent) : Path.moreDesirable(getPath(parent), getPath(nextSibling, el));
    } else if (distFromEnd === 0) {
      parentPath = ignoreEl === previousSibling ? getPath(parent) : Path.moreDesirable(getPath(parent), getPath(previousSibling, el));
    } else if (ignoreEl === previousSibling || ignoreEl !== nextSibling && getPath(nextSibling, el).isMoreDesirableThan(getPath(previousSibling, el))) {
