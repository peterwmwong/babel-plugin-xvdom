...
class Path {
  ...
  isShorterOrEqualThan(b) {
    ...
    const aEl = a.el;
    const bEl = b.el;
    log('path', 'isShorterOrEqualThan:', `length: ${ a.id }(${ a.length }) < ${ b.id }(${ b.length })`, `elNumDynamicDescendants: ${ aEl.numContainedDynamics } < ${ bEl.numContainedDynamics }`, `numDynamicProps: ${ aEl.numDynamicProps } < ${ bEl.numDynamicProps }`);
    return 0 >= (Math.sign(a.length - b.length) || Math.sign(bEl.numContainedDynamics - aEl.numContainedDynamics) || Math.sign(aEl.numDynamicProps - +bEl.numDynamicProps));
  }
  ...
  removeReference() {
    logFunc('path', `removeReference ${ this.id } numReferencingPaths = ${ this.numReferencingPaths }`, () => {
  ...
  addReference() {
    logFunc('path', `addReference ${ this.id } numReferencingPaths = ${ this.numReferencingPaths }`, () => {
      const { el: { rootPath } } = this;
      if (rootPath && rootPath !== this) {
        log('path', `adding reference to previous rootPath ${ rootPath.id }`);
...
function getPath(el, ignoreEl, isHardReferenced) {
  return logFunc('path', `getPath ${ el.tag }`, () => el.rootPath ? el.rootPath : calcPath(el, ignoreEl, isHardReferenced), result => `[ ${ result.toString() } ]`);
}
