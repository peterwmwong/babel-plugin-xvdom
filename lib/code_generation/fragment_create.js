...
function generateAssignDynamicProp({ t, instId, statements }, domNodeExpression, { dynamic, astValueNode, astNameId }, expressionToAssignDomNode) {
  const valueCode = dynamic ? memberExpr(t, instId, dynamic.instanceValueId) : astValueNode;
...
function generateSpecElementDynamicChildCode({ t, xvdomApi, statements, instId }, childDesc, parentNodeVarId) {
  ...
  statements.push(t.expressionStatement(t.assignmentExpression('=', memberExpr(t, instId, instanceContextId), t.callExpression(xvdomApi.accessAPI('createDynamic'), [t.booleanLiteral(isOnlyChild), parentNodeVarId, memberExpr(t, instId, instanceValueId)]))));
}
...
function generateSpecCreateComponentCode(context, el, depth) {
  ...
  const propsArg = !el.props.length ? t.nullLiteral() : obj(t, el.props.reduce((acc, { astNameId, astValueNode, dynamic }) => (acc[astNameId.name] = dynamic ? memberExpr(t, instId, dynamic.instanceValueId) : astValueNode, acc), {}));

  // Create element
  // ex. _xvdomCreateComponent('div');
  let createComponentCode = t.callExpression(xvdomApi.accessAPI('createComponent'), [componentId, memberExpr(t, componentId, 'state'), propsArg, instId]);
  if (el.numDynamicProps) {
    createComponentCode = t.assignmentExpression('=', memberExpr(t, instId, el.instanceContextId), createComponentCode);
  }
  ...
  if (depth) {
    statements.push(t.expressionStatement(t.callExpression(memberExpr(t, tmpVars[depth - 1].id, 'appendChild'), [tmpVars[depth].id])));
  }
}
...
function generateSpecCreateHTMLElementCode(context, el, depth) {
  ...
  // Create element
  // ex. _xvdomEl('div');
  const createCode = t.callExpression(xvdomApi.accessAPI('el'), [t.stringLiteral(el.tag)]);
  ...
  let i = 0;
  for (let prop of el.props) {
    if (shouldGenerateDynamicPropCode || !prop.dynamic) {
      // Optimization: Only assign the instanceContextId once
      generateAssignDynamicProp(context, nodeVarId, prop, i === 0 && prop.dynamic && memberExpr(t, context.instId, prop.dynamic.instanceContextId));
    }
  }
...
function sortDynamicsByDepthThenDistanceFromEnds(dynamics) {
  return dynamics.reduce((depthGroups, d) => {
...
function generateMemberExpression(t, { tmpVar, path }) {
  return !path ? tmpVar : t.assignmentExpression('=', t.identifier(tmpVar.name), memberExpr(t, ...path.map(p => expressionForRelationOrAssign(t, p))));
}

function getTmpVarForNode(t, savedNodesToTmpVarId, el) {
  let tmpVar = savedNodesToTmpVarId.get(el);
  return tmpVar || (tmpVar = tmpVarId(t, savedNodesToTmpVarId.size), savedNodesToTmpVarId.set(el, tmpVar), tmpVar);
}
...
function generateSpecCreateCloneableDynamicPropCode(context, dynamic, savedNodesToTmpVarId) {
  ...
  if (!path.parent) {
    ...
    while (path.parent && !originalSavedNodes.has(el = path.el)) {
      ...
      pathMembers = [...(savedNodesToTmpVarId.has(parentEl) ? [savedNodesToTmpVarId.get(parentEl).name] : path.parent.parent ? EMPTY_ARRAY : [getTmpVarForNode(t, savedNodesToTmpVarId, parentEl).name]), el.relationshipTo(parentEl)];
  ...
  generateAssignDynamicProp(context, generateMemberExpression(t, rootAssignExpr), dynamic.prop, !originalSavedNodes.has(dynamic.el) && dynamic.prop.dynamic && memberExpr(t, context.instId, dynamic.instanceContextId));
}
...
function generateSpecCreateCloneableElementCode(context, el, isCloneable, dynamics) {
  ...
  const createSpecContext = Object.assign({}, context, {
  ...
  const createSpecNodeId = xvdomApi.definePrefixed('xvdomSpecNodeCreate', t.functionExpression(null, EMPTY_ARRAY, t.blockStatement([t.variableDeclaration('var', createSpecContext.tmpVars), ...createSpecContext.statements, t.returnStatement(createSpecContext.tmpVars[0].id)]))).name;

  // var _n = (_xvdomSpecNode || (_xvdomSpecNode = _xvdomSpecNodeCreate())).cloneNode(true);
  tmpVars.push(t.variableDeclarator(rootElId, t.callExpression(memberExpr(t, t.logicalExpression('||', specNodeId, t.assignmentExpression('=', specNodeId, t.callExpression(createSpecNodeId, EMPTY_ARRAY))), t.identifier('cloneNode')), [t.booleanLiteral(true)])));

  generateSpecCreateCloneableDynamicCode(context, el, rootElId, dynamics);
}

function generateSpecCreateElementCode(context, el) {
  if (el instanceof JSXHTMLElement) generateSpecCreateHTMLElementCode(context, el, 0);else generateSpecCreateComponentCode(context, el, 0);
}
...
function generateSpecCreateCode(t, xvdomApi, { rootElement, dynamics, hasComponents, isCloneable }) {
  ...
  if (isCloneable) generateSpecCreateCloneableElementCode(context, rootElement, isCloneable, dynamics);else generateSpecCreateElementCode(context, rootElement);
  ...
  return t.functionExpression(null, params, t.blockStatement([t.variableDeclaration('var', context.tmpVars), ...context.statements, t.returnStatement(context.tmpVars[0].id)]));
}
