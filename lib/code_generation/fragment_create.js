...
function dynamicArrayOrJSXChildStatement(funcGen, jsxValueElement, parentNodeVarId) {
  ...
  return t.expressionStatement(t.assignmentExpression('=', funcGen.accessContext(instanceContextId), t.callExpression(funcGen.accessAPI(`createDynamic${ isOnlyChild ? 'OnlyChild' : '' }`), [parentNodeVarId, memberExpr(t, instId, instanceValueId)])));
}
...
function componentCode(funcGen, jsxElement, depth) {
  ...
  let createComponentCode = t.callExpression(funcGen.accessAPI('createComponent'), [componentId, memberExpr(t, componentId, 'state'), propsArg]);

  if (numDynamicProps) {
    createComponentCode = t.assignmentExpression('=', funcGen.accessContext(instanceContextId), createComponentCode);
  }
...
function htmlElementCode(funcGen, jsxHTMLElement, depth, shouldGenerateDynamicPropCode) {
  ...
  for (let prop of jsxHTMLElement.props) {
    if (shouldGenerateDynamicPropCode || !prop.dynamic) {
      ...
      funcGen.addStatement(dynamicPropStatement(funcGen, nodeVarId, prop, i === 0 && prop.dynamic && funcGen.accessContext(prop.dynamic.instanceContextId)));
    }
    i++;
  };
