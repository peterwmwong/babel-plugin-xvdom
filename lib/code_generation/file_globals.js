...
class FileGlobals {
  ...
  getOrSet(id, value) {
    const { globals } = this;
    return (globals[id] || (globals[id] = { value, name: this.file.scope.generateUidIdentifier(id) })).name;
  }
  ...
  accessAPI(apiFuncName) {
    return this.getOrSet(apiVarName(apiFuncName), memberExpr(this.t, 'xvdom', apiFuncName));
  }
}
