...
function dynamicArrayOrJSXChildStatement(t, xvdomApi, instId, pInstId, getTmpVar, dynamicChild) {
  ...
  return t.expressionStatement(t.assignmentExpression('=', memberExpr(t, pInstId, instanceContextId), t.callExpression(xvdomApi.accessAPI(`updateDynamic${ isOnlyChild ? 'OnlyChild' : '' }`), [memberExpr(t, pInstId, instanceValueId), t.assignmentExpression('=', memberExpr(t, pInstId, instanceValueId), memberExpr(t, instId, instanceValueId)), memberExpr(t, pInstId, instanceContextId)])));
}

function dynamicTextChildStatements(t, xvdomApi, instId, pInstId, getTmpVar, dynamicChild) {
  const { instanceContextId, instanceValueId, isOnlyChild } = dynamicChild;
  return t.expressionStatement(t.assignmentExpression('=', isOnlyChild ? memberExpr(t, pInstId, instanceContextId, 'innerText') : memberExpr(t, pInstId, instanceContextId, 'data'), t.callExpression(xvdomApi.accessAPI(`text`), [t.assignmentExpression('=', memberExpr(t, pInstId, instanceValueId), memberExpr(t, instId, instanceValueId))])));
}
...
function componentDynamicStatement(t, xvdomApi, instId, pInstId, component) {
  ...
  return t.ifStatement(condition, t.expressionStatement(t.assignmentExpression('=', memberExpr(t, pInstId, component.instanceContextId), t.callExpression(xvdomApi.accessAPI('updateComponent'), [t.identifier(component.tag), memberExpr(t, component.tag, 'state'), obj(t, component.props.reduce((hash, p) => {
...
module.exports = function (t, xvdomApi, { dynamics, customElementsWithDynamicProps }) {
  ...
  const updateDynamicPropCode = dynamics.filter(nonComponentPropDynamic).reduce((acc, dynamic) => [...acc, ...(dynamic instanceof DynamicProp ? dynamicPropStatements(t, xvdomApi, instId, pInstId, getTmpVar, dynamic) : dynamicChildStatements(t, xvdomApi, instId, pInstId, getTmpVar, dynamic))], []);
