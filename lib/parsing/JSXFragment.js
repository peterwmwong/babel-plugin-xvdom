...
const hasSideEffects = (tag, propName) => tag === 'input' && propName === 'value' || tag === 'a' && propName === 'href';
...
class JSXElementProp {
  constructor(fragment, astNode, attr, el) {
    ...
    this.dynamic = isDynamicNode(t, this.astValueNode) && fragment.addDynamicProp(this);
  }
}
...
class JSXElementChild {
  ...
  get distFromEnd() {
    return !this.parent ? 0 : this.parent.children.length - this.index - 1;
  }
  get firstChild() {
  ...
  relationshipTo(comp) {
    return this === comp.firstChild ? 'firstChild' : this === comp.lastChild ? 'lastChild' : this === comp.nextSibling ? 'nextSibling' : this === comp.previousSibling ? 'previousSibling' : 'UNKNOWN';
  }
}
...
class JSXElement extends JSXElementChild {
  constructor(fragment, astJSXElement, parent) {
    ...
    this.props = openingElement.attributes.filter(isNotXvdomAttribute).map(attr => new JSXElementProp(fragment, astJSXElement, attr, this));
  }
...
class JSXHTMLElement extends JSXElement {
  constructor(fragment, astJSXElement, parent) {
    ...
    this.children = filteredChildren.map((child, i) => t.isJSXElement(child) ? createJSXElement(fragment, child, this) : new JSXValueElement(fragment, child, this, hasOnlyOneChild, i === 0));
  }
}

const createJSXElement = (fragment, astJSXElement, parent) => !isComponentName(astJSXElement.openingElement.name.name) ? new JSXHTMLElement(fragment, astJSXElement, parent) : new JSXElement(fragment, astJSXElement, parent);
...
class DynamicProp extends Dynamic {
  ...
  get hasSideEffects() {
    return this.prop.hasSideEffects;
  }
  get astNameId() {
    return this.prop.astNameId;
  }
  get astValueNode() {
    return this.prop.astValueNode;
  }
}
...
class DynamicChild extends Dynamic {
  ...
  get isText() {
    return this.child.isText;
  }
  get isOnlyChild() {
    return this.child.isOnlyChild;
  }
  get isFirstChild() {
    return this.child.isFirstChild;
  }
  get astValueNode() {
    return this.child.astValueNode;
  }
}
...
class JSXFragment {
  ...
  get hasComponents() {
    ...
    while (el = els.pop()) {
      ...
      } else if (el instanceof JSXElement) {
        return true;
      }
    }
    return false;
  ...
  addDynamicChild(parent) {
    const dynamic = new DynamicChild(this._genInstancePropId(), this._genInstancePropId(), parent);
  ...
  addDynamicProp(prop) {
    const { el } = prop;
    const dynamic = new DynamicProp(this._getInstanceContextIdForNode(el), this._genInstancePropId(), prop);
