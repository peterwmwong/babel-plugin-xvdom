...
function createComponentCode(t, elCompName, props, instanceParamId, dynamicIds) {
  ...
  return t.callExpression(t.memberExpression(t.identifier("xvdom"), t.identifier("createComponent")), [t.identifier(elCompName), componentObjProps.length ? t.objectExpression(componentObjProps) : t.nullLiteral(), instanceParamId, t.stringLiteral(dynamicIds.rerenderId.name), t.stringLiteral(dynamicIds.contextId.name), t.stringLiteral(dynamicIds.componentId.name)]);
}
...
function createRerenderStatementForComponent(t, dyn, instanceParamId, prevInstanceParamId) {
  ...
  })), t.nullLiteral(), t.memberExpression(prevInstanceParamId, dyn.componentId), t.memberExpression(prevInstanceParamId, dyn.contextId), prevInstanceParamId, t.stringLiteral(dyn.contextId.name), t.stringLiteral(dyn.componentId.name)]))].concat(_toConsumableArray(dynamicProps.map(function (prop) {
...
function createSpecObject(t, file, genDynamicIdentifiers, desc) {
  ...
  var specProperties = objProps(t, {
    ...
    r: desc.recycle ? t.callExpression(t.memberExpression(xvdomId, t.identifier("Pool")), EMPTY_ARRAY) : t.memberExpression(xvdomId, t.identifier("DEADPOOL"))
  });
...
function createInstanceObject(t, file, desc) {
  ...
  var instancePropsForDynamics = genDynamicIdentifiers.dynamics.reduce(function (props, _ref3) {
    ...
    return [].concat(_toConsumableArray(props), _toConsumableArray(valueId ? [objProp(t, valueId, value)] : EMPTY_ARRAY), _toConsumableArray(isComponent ? instancePropsForComponent(t, { componentPropMap: componentPropMap, componentId: componentId }) : EMPTY_ARRAY));
  }, []);

  var objectProps = [objProp(t, "$s", specObject)].concat(_toConsumableArray(instancePropsForDynamics));
