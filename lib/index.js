...
function createElementsCode(t, instanceParamId, genUidIdentifier, genDynamicIdentifiers, parentId, children) {
  ...
  return children.reduce(function (acc, child) {
    ...
    if (t.isJSXElement(child)) {
      ...
      createEl = t.callExpression(t.memberExpression(t.identifier("xvdom"), t.identifier("createDynamic")), [children.length === 1 ? parentId : t.identifier("null"), t.memberExpression(instanceParamId, valueId), instanceParamId, t.stringLiteral(rerenderId.name), t.stringLiteral(contextId.name)]);
    } else {
...
function createSpecObject(t, file, genDynamicIdentifiers, desc) {
  ...
  var specProperties = [objProp(t, "c", createRenderFunction(t, genDynamicIdentifiers, desc))];
  var dynamics = genDynamicIdentifiers.dynamics.filter(function (dyn) {
    return !dyn.isComponent || dyn.hasDynamicComponentProps;
  });

  specProperties.push(objProp(t, "u", dynamics.length ? createRerenderFunction(t, dynamics) : t.functionExpression(null, EMPTY_ARRAY, t.blockStatement(EMPTY_ARRAY))));
...
function createInstanceObject(t, file, desc) {
  ...
  function genDynamicIdentifiers(value, prop, componentName, componentProps, contextId) {
    ...
    var result = {
      ...
      valueId: isComponent ? null : t.identifier((0, _genId2.default)(lastDynamicUidInt++)),
      rerenderId: t.identifier((0, _genId2.default)(lastDynamicUidInt++)),
      contextId: contextId || t.identifier((0, _genId2.default)(lastDynamicUidInt++)),
      componentId: t.identifier((0, _genId2.default)(lastDynamicUidInt++))
    };
  ...
  var objectProps = [objProp(t, "$s", specObject), objProp(t, "$n", t.identifier("null")), objProp(t, "$c", t.identifier("null")), objProp(t, "$t", t.identifier("null")), objProp(t, "$a", t.identifier("null")), objProp(t, "$p", t.identifier("null"))].concat(_toConsumableArray(instancePropsForDynamics));
