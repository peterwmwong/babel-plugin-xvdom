...
function createPropsStatements(t, instanceParamId, genDynamicIdentifiers, nodeId, props) {
  ...
  return Object.keys(props).reduce(function (acc, prop) {
    ...
    return [].concat(_toConsumableArray(acc), _toConsumableArray(dyn ? [t.expressionStatement(t.assignmentExpression("=", t.memberExpression(instanceParamId, dyn.contextId), nodeId))] : []), [t.expressionStatement(t.assignmentExpression("=", t.memberExpression(nodeId, t.identifier(prop)), rhs))]);
  }, []);
}
...
function createSpecObject(t, file, genDynamicIdentifiers, desc) {
  ...
  specProperties.push(objProp(t, "rerender", dynamics.length ? createRerenderFunction(t, dynamics) : t.functionExpression(null, [], t.blockStatement([]))));
...
function instancePropsForComponent(t, _ref2) {
  ...
  return [objProp(t, componentId, t.identifier("null"))].concat(_toConsumableArray(!componentPropMap ? [] : Object.keys(componentPropMap).filter(function (prop) {
...
function createInstanceObject(t, file, desc) {
  ...
  var instancePropsForDynamics = dynamics.reduce(function (props, _ref3) {
    ...
    return [].concat(_toConsumableArray(props), _toConsumableArray(valueId ? [objProp(t, valueId, value)] : []), [objProp(t, rerenderId, nullId), objProp(t, contextId, nullId)], _toConsumableArray(isComponent ? instancePropsForComponent(t, { componentPropMap: componentPropMap, componentId: componentId }) : []));
  }, []);
