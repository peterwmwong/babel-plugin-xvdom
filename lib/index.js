...
function generateSpecCreateHTMLElementCode(context, el, _) {
  ...
  el.children.forEach(function (childDesc) {
    switch (childDesc.type) {
      case 'component':
        return generateSpecCreateComponentCode(context, childDesc, tmpVar, depth);
      case 'el':
        return generateSpecCreateHTMLElementCode(context, childDesc, tmpVar, depth);
      case 'dynamicChild':
        return generateSpecElementDynamicChildCode(context, childDesc, tmpVar, depth);
      case 'staticChild':
        return generateSpecElementStaticChildCode(context, childDesc, tmpVar, depth);
    }
  });
}

function generateSpecCreateElementCode(context, el) {
  return el.type === 'el' ? generateSpecCreateHTMLElementCode(context, el, null, 0) : generateSpecCreateComponentCode(context, el, null, 0);
}
...
function generateSpecUpdateCode(t, xvdomApi, _ref9) {
  ...
  var hasDynamics = dynamics.length || componentsWithDyanmicProps.size;
  var nonComponentDynamics = dynamics.filter(function (d) {
    return d.type !== 'componentProp';
  });
  var params = hasDynamics ? [instId, pInstId] : EMPTY_ARRAY;
  var statements = !hasDynamics ? [] : [t.variableDeclaration('var', [t.variableDeclarator(tmpVar)])].concat(nonComponentDynamics.reduce(function (acc, dynamic) {
    acc.push.apply(acc, generateSpecUpdateDynamicCode(t, xvdomApi, instId, pInstId, tmpVar, dynamic));
    return acc;
  }, []), Array.from(componentsWithDyanmicProps.keys()).map(function (component) {
...
module.exports = function Plugin(_ref16) {
  var t = _ref16.types;
