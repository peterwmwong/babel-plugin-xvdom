...
function createElementsCode(t, instanceParamId, genUidIdentifier, genDynamicIdentifiers, parentId, children) {
  ...
  return children.reduce(function (acc, child) {
    ...
    if (t.isJSXElement(child)) {
      ...
      var createDynamicMethod = isOnlyChild ? "createDynamicOnlyChild" : "createDynamic";

      // >>> xvdom.createDynamic(inst.v0, inst, "r0", "c0");
      createEl = t.callExpression(t.memberExpression(t.identifier("xvdom"), t.identifier(createDynamicMethod)), [].concat(_toConsumableArray(isOnlyChild ? [parentId] : []), [t.memberExpression(instanceParamId, valueId), instanceParamId, t.stringLiteral(rerenderId.name), t.stringLiteral(contextId.name)]));
    } else {
...
function createSpecObject(t, file, genDynamicIdentifiers, desc) {
  ...
  var specProperties = objProps(t, {
    ...
    r: desc.recycle ? t.objectExpression([]) : t.identifier("null")
  });
...
function createInstanceObject(t, file, desc) {
  ...
  var instancePropsForDynamics = genDynamicIdentifiers.dynamics.reduce(function (props, _ref3) {
    ...
    return [].concat(_toConsumableArray(props), _toConsumableArray(valueId ? [objProp(t, valueId, value)] : EMPTY_ARRAY), _toConsumableArray(rerenderId ? [objProp(t, rerenderId, nullId)] : EMPTY_ARRAY), _toConsumableArray(!usedContextIds[contextId.name] ? (usedContextIds[contextId.name] = true, [objProp(t, contextId, nullId)]) : EMPTY_ARRAY), _toConsumableArray(isComponent ? instancePropsForComponent(t, { componentPropMap: componentPropMap, componentId: componentId }) : EMPTY_ARRAY));
  }, []);

  var objectProps = [].concat(_toConsumableArray(objProps(t, {
    $s: specObject,
    $n: t.identifier("null"),
    $c: t.identifier("null"),
    $t: t.identifier("null"),
    $a: t.identifier("null"),
    $p: t.identifier("null")
  })), _toConsumableArray(instancePropsForDynamics));
