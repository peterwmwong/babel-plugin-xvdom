...
var REF_TO_TAG = ['a', 'b', 'div', 'i', 'input', 'span', 'table', 'tbody', 'td', 'tr'];
...
function dynamicBytecode(t, context, d) {
  ...
  updateAcc.push(t.numericLiteral(RERENDER_BYTECODE_CHILD), t.numericLiteral(context.contextNodeOffset++));
...
function elChildBytecode(t, child, context) {
  switch (child.type) {
    ...
    case 'dynamic':
      return dynamicBytecode(t, context, child);
    case 'component':
...
function elPropsBytecode(t, context, props, staticProps) {
  ...
  props.forEach(function (_ref2) {
    ...
    if (isDynamic) {
      ...
      updateAcc.push(t.numericLiteral(RERENDER_BYTECODE_ELPROP), t.numericLiteral(nameOffset << 16 | contextNodeOffset));
    } else {
