...
function createComponentCode(t, elCompName, props, instanceParamId, dynamicIds) {
  ...
  return t.callExpression(dynamicIds.createComponentId, [t.identifier(elCompName), t.memberExpression(t.identifier(elCompName), t.identifier('state')), componentObjProps.length ? t.objectExpression(componentObjProps) : t.nullLiteral(), instanceParamId, t.stringLiteral(dynamicIds.rerenderId.name), t.stringLiteral(dynamicIds.componentId.name)]);
}
...
function createElementsCode(t, instanceParamId, genUidIdentifier, genDynamicIdentifiers, parentId, children) {
  ...
  return children.reduce(function (acc, child) {
    ...
    if (t.isJSXElement(child)) {
      ...
      createEl = t.callExpression(createDynamicId, [t.booleanLiteral(isOnlyChild), parentId, t.memberExpression(instanceParamId, valueId), instanceParamId, t.stringLiteral(rerenderId.name), t.stringLiteral(contextId.name)]);
    } else {
...
function createRootElementCode(t, instanceParamId, genUidIdentifier, genDynamicIdentifiers, _ref) {
  ...
  // >>> document.createElement('div')
  var createEl = isComponent ? createComponentCode(t, el, props, instanceParamId, genDynamicIdentifiers(null, null, el, props, 'PREVENT')) : t.callExpression(t.memberExpression(t.identifier('document'), t.identifier('createElement')), [t.stringLiteral(el)]);
...
function createRerenderStatementForComponent(t, dyn, instanceParamId, prevInstanceParamId) {
  ...
  })), t.memberExpression(prevInstanceParamId, dyn.componentId), prevInstanceParamId, t.stringLiteral(dyn.componentId.name)]))].concat(dynamicProps.map(function (prop) {
