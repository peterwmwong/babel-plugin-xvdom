...
function createComponentCode(t, elCompName, props, instanceParamId, dynamicIds) {
  ...
  return t.callExpression(t.memberExpression(t.identifier('xvdom'), t.identifier('createComponent')), [t.identifier(elCompName), t.memberExpression(t.identifier(elCompName), t.identifier('state')), componentObjProps.length ? t.objectExpression(componentObjProps) : t.nullLiteral(), instanceParamId, t.stringLiteral(dynamicIds.rerenderId.name), t.stringLiteral(dynamicIds.componentId.name)]);
}
