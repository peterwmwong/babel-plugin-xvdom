...
module.exports = function(t, valuesHeadId, dynamics){
  ...
  return t.functionExpression(
    null,
    valueNames.sort().map(n => t.identifier(n)),
    t.blockStatement([
      t.ifStatement(
        t.binaryExpression('===', valuesHeadId, t.nullLiteral()),
        t.returnStatement(
          t.objectExpression([
            ...dynamics.map(d =>
              objProp(t, d.instanceValueId, d.instanceValueId, false, true)
            ),
            objProp(t, NEXT_PROP_NAME, t.nullLiteral())
          ])
        )
      ),
      ...
      ...[
        t.assignmentExpression('=',
          valuesHeadId,
          memberExpr(t, resultId, NEXT_PROP_NAME)
        ),
        ...dynamics.map(d =>
          t.assignmentExpression('=',
            memberExpr(t, resultId, d.instanceValueId),
            d.instanceValueId
          )
        )
