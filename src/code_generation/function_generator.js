...
module.exports = class FunctionGenerator {
  ...
  get code(){
    ...
    const tmpVarDeclarators = [
      ...(_instId ? [ t.variableDeclarator(_instId, memberExpr(t, 'inst', 'v')) ] : EMPTY_ARRAY),
      ...[...contextVars].map(id =>
    ...
    return t.functionExpression(
      null,
      (_instId ? [ t.identifier('inst') ] : EMPTY_ARRAY),
      t.blockStatement([
          ...
          contextVars.size
            ? [t.expressionStatement(
                t.assignmentExpression('=',
                  memberExpr(t, 'inst', 'c'),
                  t.objectExpression(
                    [...contextVars].map(v => objProp(t, v, v, false, true))
                  )
                )
              )]
            : EMPTY_ARRAY
        ...
        ...(
          _instId
            ? [t.expressionStatement(
                t.assignmentExpression('=',
                  memberExpr(t, firstTmpVar, 'xvdom'),
                  t.identifier('inst')
                )
              )]
            : EMPTY_ARRAY
